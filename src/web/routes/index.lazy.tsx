...
function Index() {
  ...
  const [editableUser, setEditableUser] = React.useState<number | undefined>(undefined)
      ...
      <div>
        ...
        <div>
          ...
          <div className="flex flex-wrap mt-3 gap-3">
            {users?.map((u: any) => (
              <div className="flex max-w-xs items-center gap-3 bg-card border border-border rounded px-1 py-[0.1rem]">
                <div className="w-full">
                  <p className="w-20 truncate"><span className="text-xs pr-1">{u.id}</span>{u.name}</p>
                </div>
                <div className="flex">
                  <button
          ...
          { editableUser &&
            <div className="fixed inset-0 w-full h-screen flex flex-col items-center justify-center bg-black backdrop-blur bg-opacity-10">
              <div className="flex gap-2 items-center">
                ...
                <button
                  onClick={() => {
                    updateUser({ id: editableUser, newName: newName as string })
