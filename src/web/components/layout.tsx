...
const CustomTopBar = () => {
  ...
  function toggleColorMode() {
    ...
    const currentTheme = body.classList.value.split(' ').find(className => themes.includes(className));
    themes.forEach(theme => { body.classList.remove(theme); });
    const nextTheme = currentTheme ? themes[(themes.indexOf(currentTheme) + 1) % themes.length] : themes[0];
  ...
  return (
    <div className="flex w-full h-10 items-center">
      <div id="drag-region" className="flex items-center px-3 w-full">
        <span className="font-bold">Juu <span className="font-extralight text-sm">{appVer}</span></span>
      </div>
      <div className="flex mr-3">
        <button className="flex items-center gap-[.1rem] mr-2 px-2 py-1 rounded-md hover:bg-bgHover" onClick={toggleColorMode}>
...
export default function Layout({ children }: LayoutProps) {
  ...
  React.useEffect(() => {
    ...
    if(router.state.location.pathname.toString().includes('renderer/index.html')){
      router.navigate({ to: "/" })
    }
  }, [])
